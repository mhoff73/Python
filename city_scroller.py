...
class Building():
    ...
    """    
    def __init__(self, x_point, y_point, width, height, color, speed):
...
class Scroller(object):
    ...
    def add_buildings(self):
        ...
        while x_location < self.width:
            ...
            x_location += self.buildings[len(self.buildings)-1].width
            self.scroll_end = x_location
    ...
    def add_building(self, x_location):
        ...
        y_location = random.randint(10, self.height)
        height = self.base - y_location
        
        building = Building(x_location, y_location, width, height, self.color, self.speed)
    ...
    def move_buildings(self):
        ...
        """
        if (self.buildings[0].x_point+self.buildings[0].width) <= 0:
            self.buildings.pop(0)

        for building in self.buildings:
          building.move()

        #x location of the last building's right side
        last_building = self.buildings[len(self.buildings)-1]
        right_x_loc = last_building.x_point + last_building.width

          
        if right_x_loc <= SCREEN_WIDTH:
            self.add_building(right_x_loc)
            




FRONT_SCROLLER_COLOR = BLACK     #(0,0,30)
MIDDLE_SCROLLER_COLOR = GREEN    #(30,30,100)
BACK_SCROLLER_COLOR = RED      #(50,50,150)
BACKGROUND_COLOR = (17, 9, 89)

back_scroller = Scroller(SCREEN_WIDTH, 20, SCREEN_HEIGHT, BACK_SCROLLER_COLOR, 1)
middle_scroller = Scroller(SCREEN_WIDTH, 200, SCREEN_HEIGHT, MIDDLE_SCROLLER_COLOR, 2)
